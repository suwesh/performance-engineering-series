...
from retrieval.models import to_embedder_uat, to_reranker_uat# implementation specific imports for embedding and reranking functions
import time
import cProfile

###embedding and prompt generating function
...
def similarity_search_faqs(vector_embeddings, sentences, num_searchresults, index, texts):
    k = num_searchresults
    #search_profiler = cProfile.Profile()
    #search_profiler.enable()
    D, I =  index.search(vector_embeddings, k)
    #search_profiler.disable()
    #search_profiler.dump_stats("/home/suwesh/Projects/chatbot_kb_retrieval/profiling_evidences/controlled_faiss_similarity_search_faqs_multicore_ProcessPoolExecutor.prof")
    ...
    return results
def similarity_search_video(vector_embeddings, sentences, num_searchresults, index, texts):
    k = num_searchresults
    #search_profiler = cProfile.Profile()
    #search_profiler.enable()
    D, I =  index.search(vector_embeddings, k)
    #search_profiler.disable()
    #search_profiler.dump_stats("/home/suwesh/Projects/chatbot_kb_retrieval/profiling_evidences/controlled_faiss_similarity_search_video_multicore_ProcessPoolExecutor.prof")
    #collecting top results
    results = []
    for i in range(min(num_searchresults, len(I[0]))):
        results.append({
...
def faiss_search_manuals_chunk(q_embeddings, top_k, index, metadata):
    #search_profiler = cProfile.Profile()
    #search_profiler.enable()    
    scores, ids = index.search(q_embeddings, top_k)
    #search_profiler.disable()
    #search_profiler.dump_stats("/home/suwesh/Projects/chatbot_kb_retrieval/profiling_evidences/controlled_faiss_faiss_search_manuals_chunk_multicore_ProcessPoolExecutor.prof")
    chunks = []
...
def chatbot_kb_retriever(sentences, rag_resources):
    ...
    with ProcessPoolExecutor(max_workers=len(search_tasks)) as executor:
        futures = [executor.submit(func, *args) for func, args in search_tasks]
        faqs_results, video_results, manuals_chunk_results = [future.result() for future in futures]
    #search_profiler.disable()
    #search_profiler.dump_stats("/home/suwesh/Projects/chatbot_kb_retrieval/profiling_evidences/controlled_faiss_multicore_ProcessPoolExecutor.prof")
    search_end = time.perf_counter()
    print(f"controlled faiss internal threading FAISS searches Stage (with external multicore: ProcessPoolExecutor search): {search_end - search_start:.6f}s")
